...
<head>
    ...
    <title>Favourites | අල්මාරිය</title>
						...
						<div class="shop-menu clearfix pull-right">
							...
							if (mysqli_connect_errno()) {
								echo "Failed to connect to MySQL: " . mysqli_connect_error();
									...
									header('Location: login.php?source=favourite');
								...
								$result = mysqli_query($con, "INSERT INTO `favourite`(`cid`, `pid`, `quantity`, `color`) VALUES ('$cid','$cartAddID','$qty','$color')");
							...
							if (isset($_GET['cartRemoveID'])) {
								...
								$result = mysqli_query($con, "DELETE FROM `favourite` WHERE cid LIKE '$cid' AND pid LIKE '$cartRemoveID'");
							...
							if (isset($_GET['cartIncID'])) {
								...
								$result = mysqli_query($con, "UPDATE `favourite` SET `quantity`=(`quantity`+1) WHERE cid LIKE '$cid' AND pid LIKE '$cartIncID'");
							...
							if (isset($_GET['cartDecID'])) {
								...
								$result = mysqli_query($con, "UPDATE `favourite` SET `quantity`=IF(`quantity`=1,1,`quantity`-1) WHERE cid LIKE '$cid' AND pid LIKE '$cartDecID'");
            ...
            <table class="table table-condensed">
                ...
                <tbody>
                	...
                	<?php
							...
							$result = mysqli_query($con, "SELECT * FROM favourite WHERE cid LIKE '$cid'");
                    ...
                    <td class="cart_quantity">
                        <div class="cart_quantity_button">
                            <a class="cart_quantity_up" href="favourite.php?cartIncID='.$row['pid'].'"><i class="fa fa-plus"></i></a>
                            <input class="cart_quantity_input" type="text" name="quantity" value="'.$row['quantity'].'" autocomplete="off"
                                   size="2">
                            <a class="cart_quantity_down" href="favourite.php?cartDecID='.$row['pid'].'"><i class="fa fa-minus"></i></a>
                        </div>
                    </td>
                    <td class="cart_total">
                        <p class="cart_total_price totals">'.$rowProduct['price'] * $row['quantity'].'</p>
                    </td>
                    <td class="cart_delete" style="visibility: '; 
                    
                    $cartCheck = mysqli_query($con, "SELECT COUNT(1) FROM cart WHERE cid LIKE '$cid' AND pid LIKE '$pid'");
                   
                   
				   while ($inCart = mysqli_fetch_array($cartCheck)) {
                   if($inCart[0]>0){
                    echo "hidden";
				   }else{
				   	echo "visible";
                    ...
                    echo'">
                        <a class="cart_quantity_delete" href="cart.php?cartAddID='.$row['pid'].'&return=1"><i class="fa fa-plus"></i></a>
                    </td>
                    <td class="cart_delete">
                        <a class="cart_quantity_delete" href="favourite.php?cartRemoveID='.$row['pid'].'"><i class="fa fa-times"></i></a>
                    </td>
...
<section id="do_action" style="visibility: hidden">
    <div class="container">
        <div class="heading">
            <h3>Favourites Summary</h3>
        </div>
        <div class="row">
            ...
            <div class="col-sm-6">
                <div class="total_area">
                    <ul>
                        <li>Favourite Sub Total <span id="cart-sub"></span></li>
